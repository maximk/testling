...
test(RootDir) ->
	...
	%%
	%% MK: testoutput3 tests locale-dependent patterns, unsupported
	%%
    Testfiles0 = ["testoutput1", "testoutput2", "testoutput4",
		 "testoutput5", "testoutput6","mod_testoutput8","testoutput10"],
	...
	%%
	%% MK: no need to run twice, nothing changed
	%%
	...
	case lists:member(g,EO) of
		...
		case EO -- [accept_nonascii] of
			...
			case XMode of
			    ...
			    index ->
				%io:format("re:run(~w,~w,~w)~n",[Chal,P,ExecOpt++Xopt]),
				case re:run(Chal,P,ExecOpt++Xopt) of
				    nomatch ->
					nomatch;
				    {match, Reslist} ->
